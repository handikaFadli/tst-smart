...
@section('title', 'Daftar SLA Rules')

@section('content')
<main>
    
    <div class="pt-5 flex items-center justify-between mb-5">
        <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Daftar SLA Rules</h1>
        @if ($user->isAdmin())
        <a href="{{ route('ticket-rules.create') }}"
                    ...
                    <table class="w-full text-sm text-left">
                        <thead>
                            <tr class="bg-slate-200 dark:bg-gray-700">
                                ...
                                <th class="px-4 py-3 text-xs font-medium text-gray-600 uppercase tracking-wider">Nama Rule</th>
                                <th class="px-4 py-3 text-xs font-medium text-gray-600 uppercase tracking-wider">Kategori</th>
                                <th class="px-4 py-3 text-xs font-medium text-gray-600 uppercase tracking-wider">Priority</th>
                                <th class="px-4 py-3 text-xs font-medium text-gray-600 uppercase tracking-wider">Response (menit)</th>
                                <th class="px-4 py-3 text-xs font-medium text-gray-600 uppercase tracking-wider">Resolution (menit)</th>
                        ...
                        @foreach($rules as $i => $rule)
                            <tr class="border-b border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                <td class="px-4 py-3 text-xs text-gray-400">{{ $rules->firstItem() + $i }}</td>

                                <td class="px-4 py-3">
                                    <div class="font-medium text-gray-900">{{ $rule->nama_rule }}</div>
                                </td>

                                <td class="px-4 py-3">
                                    <span class="text-xs text-gray-600">{{ $rule->category?->nama ?? '-' }}</span>
                                </td>

                                <td class="px-4 py-3">
                                    @php
                                        $priorityColor = match ($rule->priority) {
                                    ...
                                    @endphp
                                    <span class="inline-flex items-center gap-1 px-2 py-0.5 text-xs font-medium rounded {{ $priorityColor }}">
                                        {{ ucfirst($rule->priority) }}
                                    </span>
                                </td>

                                <td class="px-4 py-3 text-sm text-gray-600">{{ $rule->response_time }}</td>

                                <td class="px-4 py-3 text-sm text-gray-600">{{ $rule->resolution_time }}</td>
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex justify-center gap-1">
                                        <a href="{{ route('ticket-rules.edit', $rule->id) }}"
                                        ...
                                        <form id="delete-form-{{ $rule->id }}" action="{{ route('ticket-rules.destroy', $rule->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button"
                                                    onclick="openDeleteModal('delete-form-{{ $rule->id }}', '{{ $rule->nama_rule }}')"
    ...
    <div class="flex flex-col md:flex-row items-center justify-between gap-4 mt-6">
        ...
        <div class="text-sm text-gray-500">
            Menampilkan
            <span class="font-semibold">{{ $rules->firstItem() ?? 0 }}</span>
            -
            <span class="font-semibold">{{ $rules->lastItem() ?? 0 }}</span>
            dari
            <span class="font-semibold">{{ $rules->total() }}</span>
            data
        </div>

        {{-- Pagination --}}
        <div>
            {{ $rules->withQueryString() }}
        </div>
