...
<div class="px-4 pt-6 pb-10 max-w-7xl mx-auto">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit SLA Rule</h1>
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Perbarui aturan SLA tiket.</p>
    </div>

    <form action="{{ route('ticket-rules.update', $ticketRule) }}" method="POST">
        @csrf
        @method('PUT')
                    ...
                    <div class="px-6 py-5 border-b border-gray-200 dark:border-gray-700">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-400">Perbarui detail aturan SLA tiket.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div class="md:col-span-2">
                                ...
                                <input type="text" name="nama_rule" value="{{ old('nama_rule', $ticketRule->nama_rule) }}" placeholder="Contoh: High Priority SLA"
                            ...
                            <div>
                                ...
                                <select name="category_id"
                                    ...
                                    @foreach($categories as $cat)
                                        <option value="{{ $cat->id }}" {{ old('category_id', $ticketRule->category_id) == $cat->id ? 'selected' : '' }}>
                                            {{ $cat->nama }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select name="priority"
                                    ...
                                    <option value="low" {{ old('priority', $ticketRule->priority) == 'low' ? 'selected' : '' }}>Low</option>
                                    <option value="medium" {{ old('priority', $ticketRule->priority) == 'medium' ? 'selected' : '' }}>Medium</option>
                                    <option value="high" {{ old('priority', $ticketRule->priority) == 'high' ? 'selected' : '' }}>High</option>
                                </select>
                                @error('priority')
                                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="number" name="response_time" value="{{ old('response_time', $ticketRule->response_time) }}" placeholder="Contoh: 60"
                            ...
                            <div>
                                ...
                                <input type="number" name="resolution_time" value="{{ old('resolution_time', $ticketRule->resolution_time) }}" placeholder="Contoh: 1440"
                                       class="block w-full px-3 py-2.5 text-sm text-gray-900 bg-gray-50 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white transition" required min="1">
                            ...
                            <div class="md:col-span-2">
                                <label class="inline-flex items-center gap-2 text-sm font-medium text-gray-700">
                                    <input type="checkbox" name="is_active" value="1" class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500" {{ old('is_active', $ticketRule->is_active) ? 'checked' : '' }}>
                                    Active
                                </label>
                                @error('is_active')
                                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                ...
                <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-6 shadow-xs">
                    ...
                    <div class="text-sm text-gray-600 space-y-2">
                        <div>
                            ...
                            <span class="font-semibold text-gray-900">{{ $ticketRule->created_at?->format('d M Y') }}</span>
                        </div>
                        <div>
                            <span class="text-gray-500">ID:</span>
                            <span class="font-semibold text-gray-900">{{ $ticketRule->id }}</span>
                        </div>
                    </div>
                </div>
            </div>
