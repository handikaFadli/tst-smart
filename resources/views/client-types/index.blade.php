...
@section('title', 'Daftar Tipe Klien')

@section('content')
<main>
    <div class="pt-5 flex items-center justify-between mb-5">
        <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Daftar Tipe Klien</h1>
                    ...
                    <table class="w-full text-sm text-left">
                        ...
                        @foreach($clientTypes as $i => $clientType)
                            <tr class="border-b border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                <td class="px-4 py-3 text-xs text-gray-400">{{ $clientTypes->firstItem() + $i }}</td>

                                <td class="px-4 py-3">
                                    <div class="font-medium text-gray-900">{{ $clientType->nama }}</div>
                                </td>

                                <td class="px-4 py-3">
                                    <span class="text-xs text-gray-600">{{ $clientType->deskripsi ? Str::limit($clientType->deskripsi, 60) : '-' }}</span>
                                </td>

                                <td class="px-4 py-3">
                                    <div class="flex justify-center gap-1">
                                        <a href="{{ route('client-types.edit', $clientType->id) }}"
                                        ...
                                        <form id="delete-form-{{ $clientType->id }}" action="{{ route('client-types.destroy', $clientType->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button"
                                                    onclick="openDeleteModal('delete-form-{{ $clientType->id }}', '{{ $clientType->nama }}')"
    ...
    <div class="flex flex-col md:flex-row items-center justify-between gap-4 mt-6">
        ...
        <div class="text-sm text-gray-500">
            Menampilkan
            <span class="font-semibold">{{ $clientTypes->firstItem() ?? 0 }}</span>
            -
            <span class="font-semibold">{{ $clientTypes->lastItem() ?? 0 }}</span>
            dari
            <span class="font-semibold">{{ $clientTypes->total() }}</span>
            data
        </div>

        {{-- Pagination --}}
        <div>
            {{ $clientTypes->withQueryString() }}
        </div>
