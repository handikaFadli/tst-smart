...
<main>
    <div class="pt-5 flex items-center justify-between mb-5">
        <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Daftar Server</h1>
                    ...
                    <table class="w-full text-sm text-left">
                        <thead>
                            <tr class="bg-slate-200 dark:bg-gray-700">
                                ...
                                <th class="px-4 py-3 text-xs font-bold text-gray-600 uppercase tracking-wider">IP Address</th>
                                <th class="px-4 py-3 text-xs font-bold text-gray-600 uppercase tracking-wider">Catatan</th>
                                <th class="px-4 py-3 text-xs font-bold text-gray-600 uppercase tracking-wider text-center">Aksi</th>
                            </tr>
                        </thead>
                        <tbody>
                        @foreach($servers as $i => $server)
                            <tr class="border-b border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                <td class="px-4 py-3 text-xs text-gray-400">{{ $servers->firstItem() + $i }}</td>
                                ...
                                <td class="px-4 py-3">
                                    <div class="font-medium text-gray-900">{{ $server->ip_address }}</div>
                                </td>

                                <td class="px-4 py-3">
                                    <span class="text-xs text-gray-600">{{ $server->catatan ? \Illuminate\Support\Str::limit($server->catatan, 60) : '-' }}</span>
                                </td>
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex justify-center gap-1">
                                        ...
                                        <form id="delete-form-{{ $server->id }}" action="{{ route('servers.destroy', $server->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button"
                                                    onclick="openDeleteModal('delete-form-{{ $server->id }}', '{{ $server->nama }}')"
    ...
    <div class="flex flex-col md:flex-row items-center justify-between gap-4 mt-6">
        ...
        <div class="text-sm text-gray-500">
            Menampilkan
            <span class="font-semibold">{{ $servers->firstItem() ?? 0 }}</span>
            -
            <span class="font-semibold">{{ $servers->lastItem() ?? 0 }}</span>
            dari
            <span class="font-semibold">{{ $servers->total() }}</span>
            data
        </div>

        {{-- Pagination --}}
        <div>
            {{ $servers->withQueryString() }}
        </div>
